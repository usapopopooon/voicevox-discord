...
- 操作パネルの接続数表示と event 名付きログによる状態確認・運用調査支援
- 操作パネルのライセンス導線による音声ライセンス・クレジット確認
...
接続/切断は `/vc` でも操作できます。スキップ、話者変更、音声設定、辞書、
状態確認、ライセンス確認は操作パネルのボタンに集約しています。

## 運用・調査

ログは `event=... fields={...}` の形式で、`guild_id`、`channel_id`、
`user_id`、`speaker_id`、`queue_length`、`latency_ms`、`trace_id` などを
可能な範囲で出力します。利用者向けには操作パネルの状態ボタンで接続状態とエンジン状態を確認できます。
...
状態、ライセンス、更新、再投稿は確認・保守系の導線として配置します。
パネル本文には、この Bot プロセスが現在接続している VC 数だけを表示します。
接続に成功したときも、ヘルプではなく統合パネルを投稿します。
...
Botは操作パネルのライセンス導線で公式サイト・規約ページ・クレジット候補への導線を提供しますが、
